...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def add_file(self):
        ...
        if path != '':
            if self.samples[0].size != 0:
                self.reset()
            # time domain
            self.samples[0], self.sampling_freq = librosa.load(path, sr=None)
            ...
            self.freqs = np.linspace(
                0, self.freq_max, self.fft_samples[0].size)
            ...
            self.playGraph.setLimits(
                xMin=0, xMax=self.samples[0].size + 30, yMin=-1, yMax=1)
    ...
    def modify_gain(self, slider_num):
        ...
        self.mediaPlayers[test].setMedia(QMediaContent(
            QUrl.fromLocalFile("/home/abdallahdarwesh/PycharmProjects/1.DSP-Tasks/2.Equalizer/.test{}.wav".format(test))))
